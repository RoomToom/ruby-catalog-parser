...
На Windows для Gmail запустити `setup_smtp.cmd`, ввести адресу відправника
та пароль застосунку Google. Потрібна двоетапна перевірка; адміністратор
організаційної скриньки може обмежувати паролі застосунків.
Налаштування зберігаються в `.smtp.local.xml`: пароль зашифрований Windows
для поточного користувача цього ПК, файл виключено з Git. Скрипт перевіряє
SMTP-вхід без надсилання листа. Повторна перевірка: `.\scripts\run.ps1 smtp-check`.
`run_lab.cmd` і worker автоматично читають цей файл; локальні налаштування
мають пріоритет над змінними SMTP середовища. Після перенесення на інший ПК
потрібно знову виконати налаштування. Отримувач за замовчуванням —
`tokarchuk.roman@chnu.edu.ua`; іншу адресу можна задати параметром
`setup_smtp.cmd -Recipient адреса`.

Запустити Redis, потім `.\scripts\run.ps1 worker` в окремому терміналі
та `.\run_lab.cmd --send-archive` для збору й надсилання архіву.

Ручне налаштування без `.smtp.local.xml`:
